...
$(document).ready(function() {
    ...
    function getTransportTypes() {
        ...
        return fetchWithAuth(url, {}).then(function(response) {
            if (response.ok) {
                ...
                console.log('Network request for transportTypes.json failed with response ' + response.status + ': ' + response.statusText);
            }
        });
        ...
        $("#paramsForm > *").each(function(index, value) {
            ...
            $(this).find("option").each(function() {
                ...
                if ($(this).val() == selectedValue) {
                    ...
                    $("#transport-params").text($("#transport-params").text() + " " + $(this).attr("id"));
                }
            });
            if ($(this).attr("type") == "number") {
                $("#transport-params").text($("#transport-params").text() + " " + $(this).val());

            }
            // $("#transport-params").text($("#transport-params").text() + " " + $(this).attr("id"));
            // $(TRANSPORT_PARAMS).append($(value).val());
        })
        $("#choose-transport-button").removeClass('bg-primary');
        $("#choose-transport-button").addClass('bg-success');
    ...
    $("#confirm-request").on("click", function() {
        ...
        let res = {
            'transport_data_id': TRANSPORT_TYPE,
            'date': DATE,
            'start': TIME_BEGIN,
            'end': TIME_END,
            'lat': PLACE[0],
        ...
        let token = "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJiYkBiYi5ydSIsInJvbGUiOiJDTElFTlQiLCJ1c2VySWQiOjMsImlhdCI6MTY2NTg5NDIwOCwiZXhwIjoxNjY2NDk5MDA4fQ.9twmzDWEja9TFwv8HhXu7Imoh8ssDD1o3_pwUSh42Bg";
        var xhr = new XMLHttpRequest();
        // xhr.contentHeader()
        var url = "http://192.168.0.100:8080/api/requests/save";
